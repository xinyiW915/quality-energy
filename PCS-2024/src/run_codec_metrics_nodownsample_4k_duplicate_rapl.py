...
def measure_command_runtime_rapl(cmd, codec, video, coded, qp):
    ...
    try:
        ...
        if power_meter_result.returncode == 0:
            ...
            rapl_path = '/home/um20242/quality-energy-master/videocodec_downsample_4k/rapl_power/'
...
def convert_mkv_to_yuv(video_name, video_path, original_video, pixfmt):
    ...
    cmd_convert = f'ffmpeg -y -i {video_path + video_name} -c:v rawvideo -video_size {video_width}x{video_height} -pixel_format {pixfmt} {original_video}'
...
def encoding_video(codec, video, qp, framerate, video_width, video_height, pixfmt, original_video, encode_path, exp_type):
    ...
    if codec == 'x265' or codec == 'x264' or codec == 'VP9' or codec == 'SVT-AV1':
        ...
        if exp_type == 'energy':
            cmd_encode = f'sudo ./AppPowerMeter ffmpeg -video_size {video_width}x{video_height} -r {framerate} -pix_fmt {pixfmt} -y -i {original_video} -c:v {codec_name}' \
                         f' -crf {str(qp)} -video_size {video_width}x{video_height} -r {framerate} -pix_fmt {pixfmt} {encoded_video} 1>{encoded_video_log} 2>&1'
        elif exp_type == 'quality':
            cmd_encode = f'ffmpeg -video_size {video_width}x{video_height} -r {framerate} -pix_fmt {pixfmt} -y -i {original_video} -c:v {codec_name}' \
                         f' -crf {str(qp)} -video_size {video_width}x{video_height} -r {framerate} -pix_fmt {pixfmt} {encoded_video} 1>{encoded_video_log} 2>&1'
    elif codec == 'VVC':
        ...
        cmd_encode = f'vvencapp -video_size {video_width}x{video_height} -r {framerate} -c {pixfmt_c} -i {original_video} --preset medium -q {str(qp)} -o {encoded_video} 1>{encoded_video_log} 2>&1'
...
def compute_vmaf(video, qp, vmaf_path, codec, video_width, video_height, framerate, pixfmt, original_video,
    ...
    vmaf_name = f'{vmaf_path}{codec}/{video}_decoded_crf_{str(qp)}_2160p.csv'
...
def get_video_frame_count(video_path, video_type):
    if video_type == 'mkv':
        cmd_get_frame = f'ffprobe -v error -count_frames -select_streams v:0 -show_entries stream=nb_read_frames -of default=noprint_wrappers=1 {raw_video}'
    elif video_type == 'mp4':
        cmd_get_frame = f'ffprobe -v error -select_streams v:0 -show_entries stream=nb_frames -of default=noprint_wrappers=1 {video_path}'
    logger.debug(f'running ffmpeg with command: {cmd_get_frame}')

    process = subprocess.Popen(cmd_get_frame, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if process.returncode != 0:
        ...
        if match:
            frame_count = match.group(0)
            print(f"Extracted nb_frames: {frame_count}")
        else:
            print("Number not found in the output.")
            frame_count = None
...
if __name__ == '__main__':
    ...
    ugcdata_ori_path = '/home/um20242/quality-energy-master/ugc-dataset/2160P/'
    ugcdata_duplicate_path = '/home/um20242/quality-energy-master/videocodec_4k/duplicate/'
    encode_path = '/home/um20242/quality-energy-master/videocodec_4k/encoded/'
    decode_path = '/home/um20242/quality-energy-master/videocodec_4k/decoded/'
    ffmpeg_path = '/home/um20242/quality-energy-master/videocodec_4k/ffmpeg/'
    original_path = '/home/um20242/quality-energy-master/videocodec_4k/input/'
    vmaf_path = '/home/um20242/quality-energy-master/videocodec_4k/vmaf/'
    ...
    for i in range(len(ugcdata)):
        ...
        original_frame_count = get_video_frame_count(raw_video, 'mkv')
        print(original_frame_count)
        ...
        for qp in qp_level:
            ...
            # --------------------Start Encode------------------------
            cmd_encode, encoded_video, encoded_video_log = encoding_video(codec, video, qp, framerate, video_width, video_height, pixfmt, original_video, encode_path, 'quality')
            encoding_elapsed_time = measure_command_runtime(cmd_encode)
            encoding_elapsed_time_list.append(encoding_elapsed_time)
            compressed_frame_count = get_video_frame_count(encoded_video, 'mp4')
            print(compressed_frame_count)
            ...
            if original_frame_count != compressed_frame_count:
                ...
                cmd_trim = f'ffmpeg -video_size {video_width}x{video_height} -r {framerate} -pix_fmt {pixfmt} -i {decode_video} -vf select="between(n\,1\,{compressed_frame_count}),setpts=PTS-STARTPTS" {compressed_yuv}'
                subprocess.run(cmd_trim, encoding="utf-8", shell=True)
                logger.debug(f'running ffmpeg with command: {cmd_trim}')
        ...
        for index, (original_video, qp, original_width, original_height) in enumerate(result_list):
            ...
            cmd_encode, encoded_video, encoded_video_log = encoding_video(codec, original_duplicate, qp, framerate, original_width,
                                                       original_height, pixfmt, original_duplicate_yuv, encode_path, 'energy')
