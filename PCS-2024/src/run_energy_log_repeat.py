...
def test_calc_job_energy(count_repeat, encode_power, encode_time, decode_power, decode_time):
    ...
    df = pd.DataFrame(
        data={
              'start_time_encode': [start_encode], 'end_time_encode': [end_encode], 'count_repeat': [count],
              # 'target_encode': [{'start_time': start_encode, 'end_time': end_encode}],
              'start_time_decode': [start_decode], 'end_time_decode': [end_decode], 'count_repeat': [count]
...
if __name__ == '__main__':
    ...
    for i in range(len(df_metrics)):
        ...
        df_energy = test_calc_job_energy(count_repeat, encode_power, encode_time, decode_power, decode_time)
        print(df_energy.T)
    ...
    print(df_metrics.T)
    metrics_name = f'../metrics/energy/YOUTUBE_UGC_1080P_{codec}_metrics_energy_repeat.csv'
