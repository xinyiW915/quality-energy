...
if __name__ == '__main__':
    ...
    for i in range(len(ugcdata)):
        ...
        for qp in qp_level:
            ...
            psnr = calculate_psnr(codec_path, video, qp, framerate, video_width, video_height, pixfmt, original_video, encoded_video)
            psnr_list.append(psnr)

            # --------------------Start Decode------------------------
            cmd_decode, decode_video = decoding_video(codec, video, qp, decode_path, encoded_video)
            measure_command_runtime(cmd_decode)

            # --------------------VMAF computaion------------------------
            vmaf = compute_vmaf(video, qp, vmaf_path, codec, video_width, video_height, framerate, pixfmt, original_video, decode_video)
        ...
        print(df_metrics)
        metrics_name = f'../videocodec_4k/YOUTUBE_UGC_2160P_nodownsample_{codec}_metrics.csv'
    ...
    print(df_metrics)
    energy_name = f'../metrics/energy/YOUTUBE_UGC_2160P_nodownsample_{codec}_metrics_timestamp_noduplicate_distribution_energy_{category}.csv'
    ...
    df_encoding_power_name = f'../metrics/energy_log/YOUTUBE_UGC_2160P_nodownsample_{codec}_noduplicate_encoding_power_log.csv'
    ...
    df_decoding_power_name = f'../metrics/energy_log/YOUTUBE_UGC_2160P_nodownsample_{codec}_noduplicate_decoding_power_log.csv'
