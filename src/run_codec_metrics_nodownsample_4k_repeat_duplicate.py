...
if __name__ == '__main__':
    ...
    ugcdata_path = '../ugc-dataset/2160P/'
    ugcdata_duplicate_path = '../videocodec_4k/duplicate/'
    encode_path = '../videocodec_4k/encoded/'
    decode_path = '../videocodec_4k/decoded/'
    ffmpeg_path = '../videocodec_4k/ffmpeg/'
    original_path = '../videocodec_4k/input/'
    vmaf_path = '../videocodec_4k/vmaf/'
    ...
    for i in range(len(ugcdata)):
        ...
        # --------------------Convert mkv to yuv------------------------
        convert_mkv_to_yuv(video_name, ugcdata_path, original_video, pixfmt)
        ...
        for qp in qp_level:
            ...
            # --------------------Start Encode------------------------
            cmd_encode, encoded_video = encoding_video(codec, video, qp, framerate, video_width, video_height, pixfmt, original_video, encode_path)
        ...
        for index, (original_video, qp, original_width, original_height) in enumerate(result_list):
            ...
            original_duplicate_yuv = f'{duplicate_path}{original_duplicate}.yuv'

            # duplicate the video
            filter_string = f'[0:v]' * duplicate_count
            filter_complex = f'"{filter_string}concat=n={duplicate_count}:v=1[v]"'
            cmd_duplicate = f'ffmpeg -f rawvideo -pix_fmt {pixfmt} -s {original_width}x{original_height} -r {framerate} -y -i {original_video_yuv} -filter_complex {filter_complex} -map "[v]" {original_duplicate_yuv}'
            ...
            # --------------------Start Encode------------------------
            cmd_encode, encoded_video = encoding_video(codec, original_duplicate, qp, framerate, original_width,
            ...
            # --------------------Start Decode------------------------
            cmd_decode, decode_video = decoding_video_nooutput(codec, original_duplicate, qp, decode_path, encoded_video)
            ...
            # os.remove(encoded_video)
            # os.remove(decode_video)
            os.remove(original_duplicate_yuv)
        ...
        print(df_metrics)
        energy_name = f'../metrics/energy/{codec}/YOUTUBE_UGC_2160P_nodownsample_{codec}_metrics_timestamp_repeat_duplicate_distribution_energy_{video}.csv'
        ...
        df_encoding_power_name = f'../metrics/energy_log/{codec}/YOUTUBE_UGC_2160P_nodownsample_{codec}_repeat_duplicate_encoding_power_log_{video}.csv'
        ...
        df_decoding_power_name = f'../metrics/energy_log/{codec}/YOUTUBE_UGC_2160P_nodownsample_{codec}_repeat_duplicate_decoding_power_log_{video}.csv'
