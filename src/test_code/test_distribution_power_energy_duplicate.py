...
def measure_command_runtime_distribution(cmd):
    ...
    while True:
        ...
        if consecutive_failures >= 5:
            print((f'consecutive_failures: {consecutive_failures}'))
            encoding_processes[0] = (current_power_list, time_list)
            break
...
if __name__ == '__main__':
    ...
    df_metrics = pd.DataFrame(
        columns=['vid', 'QP', 'start_time_encode', 'end_time_encode', 'count_encode', 'target_encode_energy'])
    ...
    for qp in qp_level:
        video_name = 'TelevisionClip_1080P-5278'
