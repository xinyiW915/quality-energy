...
if __name__ == '__main__':
    ...
    df_cmd = pd.DataFrame(columns=['vid', 'cmd_convert', 'cmd_encode', 'cmd_psnr', 'cmd_decode', 'cmd_vmaf'])
    cmd_covert_list = []
    cmd_encode_list = []
    cmd_psnr_list = []
    cmd_decode_list = []
    ...
    for i in range(len(ugcdata)):
        ...
        print(cmd_convert)
        cmd_covert_list.append(cmd_convert)
        ...
        for qp in qp_level:
            ...
            print(cmd_encode)
            cmd_encode_list.append(cmd_encode)
            ...
            cmd_decode_list.append(cmd_decode)
    ...
    df_cmd['cmd_covert_list'] = cmd_covert_list
    df_cmd['cmd_encode_list'] = cmd_encode_list
    df_cmd['cmd_psnr_list'] = cmd_psnr_list
    df_cmd['cmd_decode_list'] = cmd_decode_list
    df_cmd['cmd_vmaf_listv'] = cmd_vmaf_list

    df_cmd_name = f'../YOUTUBE_UGC_1080P_{codec}_cmd.csv'
    df_cmd.to_csv(df_cmd_name, index=None)
